...
class Trip(object):  #rajout de object pour travailler avec super  on definit le point de recherche le plus haut
...
class Flight(Trip):
    def __init__(self, flightnum=0, departCity=None, arriveCity=None,
        ...
        self.cost = cost
        self.code = code
        super(Flight,self).__init__(departCity=departCity, arriveCity=arriveCity, #ici on ne montre plus quelle est la classe mere il la recherche
...
class Reservation():
    #notez: pas de self! on est pas vraiement un methode objet (on est une fonction utilitaire
    #interne, similaire a une methode de class en C++)
    def __nextId(number):   # le __ indique au developpeur qu'il ne doit pas modifier ...( equivlent prive en c++)
        while True:
            number = number + 1
            yield number
    __numGen = __nextId(99)

    def __init__(self, reservationId, name,
                 flightData):
        self.reservationId = next(Reservation.__numGen) #ICICICICI
        self.name = name
